...
Progress is path-keyed via the database. The playback object is the
only player contact.
"""
...
def run_rotation(
    ...
    while True:
        ...
        if code is not None:
            try:
                progress.set_offset(curr_path, playback.time_pos())
...
def _log_start(
    playback: Playback, progress: Progress, path: Path, offset: float
) -> None:
    progress.set_duration(path, playback.duration())
